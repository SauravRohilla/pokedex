...
export default function Evolution() {
  ...
  const { data: speciesData, isLoading: isLoadingSpeciesData } = useGetPokemonSpeciesQuery(
    params.id as unknown as number,
  )

  const { data, isLoading } = useGetPokemonEvolutionQuery(
    speciesData?.evolution_chain.url.slice(0, -1).split('/').at(-1) as unknown as number,
  )
            ...
            return (
              <>
                <EvolutionStage
                  ...
                  key={evo.name}
                  name={evo.name}
                  level={evo.minLevel as unknown as number}
                  id={evo.url.slice(0, -1).split('/').at(-1) as unknown as number}
                />
